...
from worker.pipeline.config import (
    ...
    R2_BUCKET_NAME, R2_PUBLIC_DOMAIN, BACKEND_WEBHOOK_URL, logger, DEEPGRAM_API_KEY
)
from worker.pipeline.tts import TTSFactory
from worker.pipeline.translation import TranslationService
        ...
        try:
            ...
            try:
                result = subprocess.run(["ffprobe", "-v", "error", "-show_entries",
                                         "format=duration", "-of",
                                         "default=noprint_wrappers=1:nokey=1", self.video_goc],
                                        check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                self.duration = float(result.stdout)
                logger.info(f"Video duration extracted: {self.duration} seconds")
            except Exception as e:
            ...
            if not self.is_translate:
                ...
                # Step 3: Transcription with Deepgram REST API
                logger.info("[2/4] Deepgram transcribing audio via REST API...")
                ...
                with open(self.audio_goc, "rb") as audio:
                    response = requests.post(deepgram_url, headers=headers, data=audio, timeout=60)
                ...
                if response.status_code == 200:
                    ...
                    if utterances:
                        ...
                        logger.warning("No utterances found from Deepgram!")
                else:
